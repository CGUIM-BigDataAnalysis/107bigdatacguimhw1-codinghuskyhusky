...
我是用X106eduatta\_salary和X103eduatta\_salary資料表的`大學-薪資`向量計算出新的yearincreaserate向量再將該向量加入X106eduatta\_salary資料表進行排序取前10項
...
這邊直接用之前的X106eduatta\_salary和新增的yearincreaserate向量篩選\>0.05的資料進行排序
...
這裡是先將每年的:大職業別，大學-女/男 以select
function取出再以`大學-女/男`\<100篩選並存在分別的4個向量中，以inner\_join
function將4個向量合併取出前10個

``` r
#這是R Code Chunk
a<-select(X106eduatta_salary,大職業別,`大學-女/男`)%>%
  filter(`大學-女/男`<100)


b<-select(X105eduatta_salary,大職業別,`大學-女/男`)%>%
  filter(`大學-女/男`<100)

c<-select(X104eduatta_salary,大職業別,`大學-女/男`)%>%
  filter(`大學-女/男`<100)

d<-select(X103eduatta_salary,大職業別,`大學-女/男`)%>%
  filter(`大學-女/男`<100)

  #arrange(desc(`經常性薪資-女/男`))

f<-inner_join(a,b,by="大職業別")%>%inner_join(c,by="大職業別")%>%inner_join(d,by="大職業別")

knitr::kable(
    head(filter(f,`大學-女/男.x`<100&`大學-女/男.x`!=0&
                  `大學-女/男.y`<100&`大學-女/男.y`!=0&
                  `大學-女/男.x.x`<100&`大學-女/男.x.x`!=0&
                  `大學-女/男.y.y`<100&`大學-女/男.y.y`!=0),10)
             )
```

| 大職業別                    | 大學-女/男.x | 大學-女/男.y | 大學-女/男.x.x | 大學-女/男.y.y |
| :---------------------- | :------: | :------: | :--------: | :--------: |
| 工業及服務業部門                |  98.36   |  98.13   |   98.06    |   97.91    |
| 工業及服務業部門-專業人員           |  98.93   |  98.49   |   98.59    |   98.67    |
| 工業及服務業部門-技術員及助理專業人員     |  98.94   |  99.02   |   98.77    |   98.72    |
| 工業及服務業部門-事務支援人員         |  98.46   |  98.02   |   98.15    |   97.82    |
| 工業及服務業部門-服務及銷售工作人員      |  99.32   |  98.96   |   98.58    |   98.56    |
| 工業及服務業部門-技藝、機械設備操作及組裝人員 |  98.39   |  98.39   |   98.01    |   97.77    |
| 工業部門                    |  98.12   |  98.01   |   97.92    |   97.66    |
| 工業部門-專業人員               |  98.66   |  98.38   |   98.33    |   98.15    |
| 工業部門-技術員及助理專業人員         |  98.58   |  98.63   |   98.50    |   98.41    |
| 工業部門-事務支援人員             |  98.44   |  98.02   |   98.29    |   97.80    |
...
礦業及土石採取業-事務支援人員CP值最高，這裡我是將X106eduatta\_salary先存入x106edua變數中，將`研究所及以上-薪資`和`大學-薪資`以as.numeric()數據化，並計算薪資增加的比例存成increaserate\_from\_UStoGU向量ㄗ在x106edua中在排序出結果
